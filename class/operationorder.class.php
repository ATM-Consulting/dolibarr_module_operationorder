...
//    public $multicurrency_total_ttc;
    ...
    public function update(User &$user, $notrigger = false)
    {
        ...
        $this->time_planned_t = $this->getTimePlannedT();
        $this->updateOperationOrderActions();
...
//                $result=$this->update_price(1, 'auto', 0, $mysoc);	// This method is designed to add line from user input so total calculation must be done using 'auto' mode.
    ...
    public function updateOperationOrderActions() {
        ...
        if($TORActions) {
            ...
            $format = 'd/m/Y H:i:s';
            $date1 = date('d/m/Y', $this->planned_date);
            $date2 = date($format, $operationorderaction->dated);
            $date2 = DateTime::createFromFormat($format, $date2);

            $date1explode = explode("/",$date1);
            $date2 = $date2->setDate($date1explode[2], $date1explode[1], $date1explode[0]);

            $operationorderaction->dated = $date2->getTimestamp();
